# This file contains algorithm to generate the output menu of recipes.
# Code in this file is mostly written by us, with the exception of the technique of 
...
# stopping conditions for while loops in the functions
MAX_TRIAL_BEFORE_GOING_TO_ALT_LIST = 2
MAX_TRIAL_BEFORE_REPEATING_INGREDIENT = 3
MAX_TRIAL_AFTER_REPEATING_INGREDIENT = 3
MAX_TRIAL_BEFORE_IGNORING_CALORIES = 10

# maximum calories for each meal as a proportion of the total calories of the day 
...
def clean_one_recipe_list(recipe_list, major_ingredients, default_cal):
    ...
    for i in range(len(recipe_list)):
        # the list consists of tuples eg. (one recipe dict, a list of ingredients used)
        item = recipe_list[i][0] 
        calories = default_cal 
        for x in item["nutritionEstimates"]:
            if x["attribute"] == "ENERC_KCAL":
                calories = x["value"]
                break
        # to clean the ingredient lines returned by API         
        if item["ingredientLines"][0][0:11] == "Ingredients": 
            # to get rid of the word "Ingredients" at the start
            ingredient_lines = [item["ingredientLines"][0][12:]] 
        else:
            # to remove repeated lines
            ingredient_lines = list(set(item["ingredientLines"])) 

        meal = Meal(item["name"], calories, item["totalTime"], recipe_list[i][1],\
                     ingredient_lines, item["images"][0]["hostedLargeUrl"], \
                     item["source"]["sourceRecipeUrl"])
...
def clean_recipes(recipe_lists):
    ...
    # use list(set()) to remove repeated recipe in the list returned by API  
        breakfast_alt_list = list(set(clean_one_recipe_list(breakfast_alt_list_old,\
                                        major_ingredients, DEFAULT_CAL_BREAKFAST)))
    else:
        breakfast_alt_list = []
    
    breakfast_list = list(set(clean_one_recipe_list(breakfast_list_old, \
                                major_ingredients, DEFAULT_CAL_BREAKFAST)))
    
    if main_dish_alt_list_old != []:
        main_dish_alt_list = list(set(clean_one_recipe_list(main_dish_alt_list_old,\
                                        major_ingredients, DEFAULT_CAL_MAIN_DISH)))
    else:
        main_dish_alt_list = []
    
    main_dish_list = list(set(clean_one_recipe_list(main_dish_list_old,\
                                major_ingredients, DEFAULT_CAL_MAIN_DISH)))
...
def pick_recipe(max_trail, recipe_list, max_calories, used_ingredients, used_recipe):
    ...
    chosen_recipe = None
    trial_count = 0
    while (trial_count < max_trail) and (chosen_recipe == None \
        or chosen_recipe == used_recipe):
        trial_count += 1
        index = random.randint(0, len(recipe_list) - 1)
        recipe = recipe_list[index]
        if recipe.calories < max_calories and \
            list(used_ingredients & set(recipe.major_ingredients)) == []: 
...
def set_meal(day, meal_type, main_list, alt_list, used_ingredients, used_recipe=None):
    ...
    chosen_recipe = None
    if main_list != []:
        chosen_recipe = pick_recipe(MAX_TRIAL_BEFORE_GOING_TO_ALT_LIST, \
            main_list, max_calories, used_ingredients, used_recipe)
        from_alt = False

    if chosen_recipe == None and alt_list != []:
        chosen_recipe = pick_recipe(MAX_TRIAL_BEFORE_REPEATING_INGREDIENT, \
    ...
    if chosen_recipe == None:
        ...
        if main_list != []:
            chosen_recipe = pick_recipe(MAX_TRIAL_AFTER_REPEATING_INGREDIENT, \
                                main_list, max_calories, set(), used_recipe) 
            from_alt = False
        else:
            if alt_list != []:
                chosen_recipe = pick_recipe(MAX_TRIAL_AFTER_REPEATING_INGREDIENT, \
                                    alt_list, max_calories, set(), used_recipe)
                from_alt = True
...
def generate_Day(breakfast_alt_list, breakfast_list, main_dish_alt_list, main_dish_list, 
    ...
    while total_trial < MAX_TRIAL_BEFORE_IGNORING_CALORIES and not day.is_qualified():
        ...
        day, used_ingredients, breakfast_from_alt = set_meal(day, \
            "breakfast", breakfast_list, breakfast_alt_list, used_ingredients)
        day, used_ingredients, lunch_from_alt = set_meal(day, \
            "lunch", main_dish_list, main_dish_alt_list, used_ingredients)
        day, used_ingredients, dinner_from_alt = set_meal(day, "dinner", \
            main_dish_list, main_dish_alt_list, used_ingredients, day.lunch)
    ...
    breakfast_list, breakfast_alt_list, main_dish_list, main_dish_alt_list = \
        update_recipe_lists(day, (breakfast_list, breakfast_alt_list, main_dish_list,\
         main_dish_alt_list), [breakfast_from_alt, lunch_from_alt, dinner_from_alt])
    return day 
...
def update_output_lists(day, output_lists, day_num, is_alt_list):
    ...
    breakfast_output_list.append({"num": day_num + 1 + additional_index, 
        "name": day.breakfast.name, "calories": day.breakfast.calories, 
        "cooking_time": day.breakfast.cooking_time, 
        "ingredients": day.breakfast.full_ingredients, 
        "pic_url": day.breakfast.pic_url, 
        "instruction_url": day.breakfast.instruction_url})
    lunch_output_list.append({"num": day_num + 8 + additional_index, 
        "name": day.lunch.name, "calories": day.lunch.calories, 
        "cooking_time": day.lunch.cooking_time, 
        "ingredients": day.lunch.full_ingredients, "pic_url": day.lunch.pic_url, 
        "instruction_url": day.lunch.instruction_url})
    dinner_output_list.append({"num": day_num + 15 + additional_index, 
        "name": day.dinner.name, "calories": day.dinner.calories, 
        "cooking_time": day.dinner.cooking_time, 
...
def generate_final_output(args_from_ui):
    ...
    if "calories_per_day" in args_from_ui:
        if len(args_from_ui["calories_per_day"]) != 2 or \
        args_from_ui["calories_per_day"][0] >= args_from_ui["calories_per_day"][1]:
            raise MyError(message="invalid calories range (must enter both upper \
                and lower limits for calories per day, with upper limit greater \
                than lower limit)")

    available_recipes = generate_available_recipes(args_from_ui)   
    breakfast_alt_list, breakfast_list, main_dish_alt_list, main_dish_list = \
        clean_recipes(available_recipes)
    ...
    for i in range(7):
        ...
        if i == 0:
            day = generate_Day(breakfast_alt_list, breakfast_list, \
                    main_dish_alt_list, main_dish_list, args_from_ui)
        elif i == 1:
            day = generate_Day(breakfast_alt_list, breakfast_list, \
                    main_dish_alt_list, main_dish_list, args_from_ui, 
                Day1 = day_list[-1])
    ...
    for i in range(7):
        ...
        if breakfast_alt_list == [] and breakfast_list == []:
            ...
            raise MyError()
        day = generate_Day(breakfast_alt_list, breakfast_list, main_dish_alt_list, \
                main_dish_list, args_from_ui)
        alternative_breakfast_list, alternative_lunch_list, alternative_dinner_list\
         = update_output_lists(day, (alternative_breakfast_list, \
            alternative_lunch_list, alternative_dinner_list), i, True)

    with open("final_output.txt", "w") as f:
        print(breakfast_final_list, ",", lunch_list, ",", dinner_list, ",", \
            calories_list, ",", alternative_breakfast_list, ",", alternative_lunch_list, \
            ",", alternative_dinner_list, file = f)
    ...
    return breakfast_final_list, lunch_list, dinner_list, calories_list, \
        alternative_breakfast_list, alternative_lunch_list, alternative_dinner_list
