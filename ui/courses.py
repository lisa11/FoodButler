...
def get_terms(args_from_ui):
    """
    If term(s) is included in the search criteria, forms a string that 
    will create a temporary table in the query containing results that 
    have the specified term(s).
    ...
    if "terms" in args_from_ui:
        temp1 = " JOIN (SELECT course_id, count(course_id) as count \
                FROM catalog_index \
                WHERE"                
        temp2 = "GROUP BY course_id HAVING count={})"
        terms = args_from_ui["terms"].lower().split()
...
def get_variables(args_from_ui):
    ...
    if args_from_ui != {}:
        rv += ["dept", "course_num"]
    if ("day" in args_from_ui or "time_start"  in args_from_ui or 
        "time_end"  in args_from_ui or "walking_time"  in args_from_ui or 
        "building"  in args_from_ui or "enroll_lower"  in args_from_ui or 
        "enroll_upper" in args_from_ui):
        rv += ["section_num", "day", "time_start", "time_end"]
    if "building" in args_from_ui:
        rv += ["sections.building_code", "TEMP2.walking_time"]
...
def get_path(variables, args_from_ui):
    ...
    if variables != []:
        tables += ["courses"]
    if ("section_num" in variables or "enrollment" in variables or 
        "day" in variables or "time_end" in variables or "time_start" in variables):
        tables += ["sections"]
    ...
    if join_conditions != [] or "terms" in args_from_ui or "building" in args_from_ui:
...
def find_eligible_buildings(args_from_ui):
    """
    If building and walk time are included in the search criteria, 
    forms a string that will create a temporary table in the query 
    containing results that match the specified criteria.
    ...
    Outputs:
        temp: a string that creates the temporary table
        arg_list: list containing building name and max walking_time 
                that will be passed on as arguments into the query
    """
    temp = ""
    arg_list = []
    if "building" in args_from_ui:
        arg_list = [args_from_ui["building"], args_from_ui["walking_time"]]
        temp = " JOIN (SELECT a.building_code, \
                compute_time_between(a.lon, a.lat, b.lon, b.lat) as walking_time \
                FROM gps as a JOIN \
                (SELECT lon, lat, building_code \
...
def write_where_string_and_args(args_from_ui):
    ...
    Outputs:
        where_string: the where string for query
        args: list of arguments to be passed onto cursor execute function
    """
    where_dict = {"dept": " dept = ?", 
                "day": " day = ?", 
                "time_start": " time_start >= ?",
                "time_end": " time_end <= ?",
                "enroll_lower": " enrollment >= ?",
                "enroll_upper": " enrollment <= ?"}

    where_string_list = []
    args = []

    for criteria in args_from_ui:
        if criteria == "day":
            s_list = []
            for day in args_from_ui[criteria]:
                args.append(day)
                s_list.append(where_dict[criteria])
            where_string_list.append(" (" + " OR".join(s_list) + ")")
        elif criteria in where_dict: 
            args.append(args_from_ui[criteria])
            where_string_list.append(where_dict[criteria])
...
def write_sql_code(args_from_ui):
    ...
    Outputs:
        complete_string: complete string for SQL query 
        args_from_ui: complete list of arguments to be passed onto 
    ...
    args += arg
    complete_string = select_string+" "+from_string+" "+on_string+" "+where_string+" \
                        ORDER BY dept, course_num"
    
    return complete_string, args
